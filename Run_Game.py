...
def run_game_with_ML(display, clock, weights):
    ...
    for _ in range(test_games):
        ...
        # while True:
            ...
            snake_position, apple_position, score, moves, count_loop = play_game(snake_start, snake_position, apple_position, button_direction, score, display, clock, moves, count_loop)

            if score > max_score:
                max_score = score

            if count_loop == len(moves):
                print("LOOP")
                break 

            # Checking condition for snake movement in loop.
            if count_same_direction > 8 and predicted_direction != 0:
                score2 -= 100 # Give a negative score to mention that its a wrong move. 
            else:
